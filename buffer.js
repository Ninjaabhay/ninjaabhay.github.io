...
async function loadPlaylists() {
  try {
    ...
    playlists.forEach((playlist, index) => {
      ...
      card.addEventListener("click", () => loadSongsFromPlaylist(playlist.id));
      ...
      if (index === 0) {
        loadSongsFromPlaylist(playlist.id);
      }
    });
  } catch (error) {
...
// Play audio function with buffering
function playAudio(idx) {
  ...
  // Buffer the song by setting preload="auto"
  currentAudio.preload = "auto";

  // Start the playback as soon as enough data is loaded
  currentAudio
    .play()
    .then(updateSeekBar)
    .catch((err) => console.error("Playback error:", err));

  // Event listeners for time updates and song end
